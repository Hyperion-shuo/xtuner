"""DAPO Math disaggregated async RL config.

Default resource layout: 8 GPUs total, 4 for training and 4 for rollout.
Required environment variables: WORK_DIR, MODEL_PATH, DATA_PATH, EVAL_DATA_PATH.
"""
...
# Keep v3 aligned with the DAPO colocate baseline: large train batch and
# multiple optimizer steps, while rollout and train workers are disaggregated.
experimental_name = "dapo_math"
train_optimizer_steps = 16
sync_weights_interval = 1
total_train_steps = 500 * sync_weights_interval
train_batch_size = 512 // sync_weights_interval
prompt_repeat_k = 16
rollout_tp_size = 1
rollout_ep_size = 1
...
evaluate_step = 5 * sync_weights_interval

max_staleness = 1
oversample_threshold = 0.2
tail_batch_trigger_size = train_batch_size
...
# 2. Rollout workers use the independent rollout resource pool.
